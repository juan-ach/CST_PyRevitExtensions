...
# CONFIGURACIÓN
# ==================================================
NOMBRE_FAMILIA_TAG_BASE = "CST_TAG Equipos Mecanicos v{}"
VERSION_FAMILIA_INICIAL = 4
VERSION_FAMILIA_FINAL = 10
...
nombre_familia_tag_usada = None

todos_los_symbols = list(
    FilteredElementCollector(doc)
    .OfClass(FamilySymbol)
    .OfCategory(BuiltInCategory.OST_MechanicalEquipmentTags)
)

for version in range(VERSION_FAMILIA_INICIAL, VERSION_FAMILIA_FINAL + 1):
    nombre_familia = NOMBRE_FAMILIA_TAG_BASE.format(version)
    candidate_base = None
    candidate_evap = None

    for fs in todos_los_symbols:
        if fs.FamilyName != nombre_familia:
            continue
    ...
    if candidate_base and candidate_evap:
        tag_symbol_base = candidate_base
        tag_symbol_evap = candidate_evap
        nombre_familia_tag_usada = nombre_familia
        break

if not tag_symbol_base or not tag_symbol_evap:
    familias_probadas = ", ".join([
        NOMBRE_FAMILIA_TAG_BASE.format(v)
        for v in range(VERSION_FAMILIA_INICIAL, VERSION_FAMILIA_FINAL + 1)
    ])
    raise Exception(
        u"No se encontraron todos los tipos de etiqueta requeridos. Familias probadas: {}.".format(
            familias_probadas)
    )
